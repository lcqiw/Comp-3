...
public:
    ...
    BSTNode(T data) : _left(nullptr), _right(nullptr) 
    {
         _data = data; 
    }
	BSTNode(BSTNode&);
    ~BSTNode(); 
	BSTNode& operator= (BSTNode&); 
...
template<typename T>
void BSTNode<T>::inOrderDisplay(ostream& out)
const {
	if (_left != nullptr) 
    {
 ...
 */
template<typename T>
void BSTNode<T>::preOrderDisplay(ostream& out) 
const {
	// comment out the next line when you've completed this function!
...
template<typename T>
void BSTNode<T>::postOrderDisplay(ostream& out)
 const {
	// comment out the next line when you've completed this function!
